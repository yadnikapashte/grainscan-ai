...
import {
  ...
  TrendingUp, AlertTriangle, CheckCircle, Info, History, Calendar, LayoutDashboard, Microscope, ShieldCheck, Database, Loader2, Brain
} from 'lucide-react'
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="inline-flex items-center gap-2 mb-4">
            ...
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${status === 'Verified' ? 'bg-status-normal/10 text-status-normal border-status-normal/20' :
        ...
        <div className="flex flex-wrap items-center gap-3">
          {profile?.role === 'Quality Inspector' && (
            <div className="flex items-center gap-2 p-1 bg-background-soft rounded-2xl border border-surface-border mr-2">
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${status === 'Verified' ? 'bg-status-normal text-white' : 'hover:bg-status-normal/10 text-status-normal'
                  }`}
              >
                <CheckCircle size={14} />
                {t('common.verify')}
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${status === 'Flagged' ? 'bg-status-discolored text-white' : 'hover:bg-status-discolored/10 text-status-discolored'
                  }`}
              >
          ...
          <button
            onClick={() => nav(`/ml-analysis?result=${activeResult.id}`)}
            className="btn-secondary py-3 text-sm flex items-center gap-2 border border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary"
          >
            <Brain size={18} />
            View AI Insights
          </button>
              ...
              <div className="flex items-center gap-2 p-1 bg-background-soft rounded-xl overflow-x-auto">
                {['All', 'Normal', 'Broken', 'Chalky', 'Discolored'].map(cat => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeFilter === cat
              ...
              {(activeResult.grains || [])
                ...
                .map((grain, idx) => (
                  ...
                  >
                    ...
                    />
                    <div className={`absolute bottom-0 left-0 right-0 py-1 text-[10px] font-black text-center uppercase tracking-tighter text-white backdrop-blur-md ${grain.quality === 'Normal' ? 'bg-status-normal/70' :
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={() => setSelectedGrain(null)}>
              <div className="card-premium max-w-2xl w-full p-8 space-y-6 relative overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex items-center gap-4 mb-4">
                  <div className={`w-3 h-10 rounded-full ${selectedGrain.quality === 'Normal' ? 'bg-status-normal' :
