...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-background grain-overlay">
      {/* Desktop Left Sidebar */}
      <aside className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0 md:left-0 bg-white border-r border-surface-border z-40 p-5 justify-between shadow-sm">
        {/* Top: Logo + Nav */}
        <div className="flex flex-col gap-8">
          {/* Logo */}
          <NavLink to="/dashboard" className="flex items-center gap-2 group px-2 py-1">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  ...
                  ${isActive
                    ...
                    : 'text-text-body border border-transparent hover:text-primary hover:bg-primary/5'
                  }`
                }
              >
        ...
        {/* Bottom: Language Switcher + Profile */}
        <div className="flex flex-col gap-4 pt-4 border-t border-surface-border">
          {/* Language Switcher */}
          <div className="relative group/lang">
            <button className="w-full flex items-center justify-between px-3 py-2 rounded-xl border border-surface-border bg-background-soft/50 text-text-body hover:bg-white hover:border-primary/30 transition-all text-xs font-bold uppercase tracking-wider">
              <span className="flex items-center gap-2">
                <Globe size={14} className="text-primary/70" />
                <span>{LANGUAGES.find(l => l.code === i18n.language.split('-')[0])?.label || 'Language'}</span>
              </span>
              <span>{LANGUAGES.find(l => l.code === i18n.language.split('-')[0])?.flag || '🇺🇸'}</span>
            </button>
            
            <div className="absolute bottom-full left-0 mb-2 w-full bg-white border border-surface-border rounded-2xl shadow-premium opacity-0 invisible group-hover/lang:opacity-100 group-hover/lang:visible transition-all z-50 p-2">
              {LANGUAGES.map((lang) => (
                <button
                  key={lang.code}
          ...
          {/* Profile Section */}
          <div className="flex items-center justify-between p-2 rounded-xl bg-background-soft/50 border border-surface-border/50 relative group/profile">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-full bg-secondary/10 border border-secondary/20 flex items-center justify-center text-secondary font-bold text-sm flex-shrink-0">
                {getInitials(profile?.full_name)}
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-sm font-bold text-text-header leading-tight truncate">
                  {profile?.full_name || 'User'}
                </span>
                <span className="text-[10px] uppercase tracking-wider text-text-body font-bold mt-0.5 truncate">
                  {profile?.role || 'Member'}
                </span>
              </div>
            </div>
            ...
            {/* Profile Dropdown */}
            <div className="absolute bottom-full left-0 mb-2 w-full bg-white border border-surface-border rounded-2xl shadow-premium opacity-0 invisible group-hover/profile:opacity-100 group-hover/profile:visible transition-all z-50 p-2">
      ...
      <div className="flex-1 flex flex-col min-w-0 md:pl-64">
        {/* Mobile Top Header */}
        <header className="md:hidden border-b border-surface-border bg-white/80 backdrop-blur-md sticky top-0 z-50">
